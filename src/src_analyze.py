...
def analyze_keywords_sentiment(df, subreddit_name):
    ...
    df_subreddit = df[df['subreddit'] == subreddit_name]
    ...
    for _, row in df_subreddit.iterrows():
        post_sent = row['post_sentiment']
        if pd.notna(row['features_in_post']):
            keywords = literal_eval(row['features_in_post'])  
            for kw in keywords: 
                feature_sentiment.setdefault(kw, []).append(post_sent) # keyword as key, sentiment as value
...
def compute_feature_rating_stats(
    ...
    df = df.copy()
    df['prod_features'] = df[feature_col].apply(lambda x: literal_eval(x) if pd.notna(x) else [])
